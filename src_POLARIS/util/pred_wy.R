...
wy_pred_fxn <- function(model, sp, model_years, bcmPath, soilPath, salinityPath, pathOut) {
    ...
    bd_POLARIS <- terra::rast(list.files(path = soilPath,
                                            pattern = "bd_0_200",
                                            full = TRUE)) %>% 
        resample(aet)
    om_POLARIS <- terra::rast(list.files(path = soilPath,
                                            pattern = "om_0_200",
                                            full = TRUE)) %>% 
        resample(aet)
    ph_POLARIS <- terra::rast(list.files(path = soilPath,
                                            pattern = "ph_0_200",
                                            full = TRUE)) %>% 
        resample(aet)
    silt_POLARIS <- terra::rast(list.files(path = soilPath,
                                              pattern = "silt_0_200",
                                              full = TRUE)) %>% 
        resample(aet)
    clay_POLARIS <- terra::rast(list.files(path = soilPath,
                                              pattern = "clay_0_200",
                                              full = TRUE)) %>% 
        resample(aet)
    ...
    stack <- rast(list(
        ...
        ppt4, ppt5, ppt6,
        ppt7, ppt8, ppt9,
        bd_POLARIS, om_POLARIS, ph_POLARIS, silt_POLARIS, clay_POLARIS, salinity
    )) %>% 
        project("EPSG:3310")
    names(stack) <- c(
        'aet', 'tmx', 'tdiff', 
