...
p10Visualize = function(sp, model){
    ...
    out.dir = paste0("data_v2/5_figs/lowFilter/comprehensive/p10_sum/")
    dir.create(out.dir, recursive=T)
    out.png.filename.CA = paste0("data_v2/5_figs/lowFilter/comprehensive/p10_sum/", sp, "_", model, "_lowFilter_sum_CA.png")
    out.png.filename.CV = paste0("data_v2/5_figs/lowFilter/comprehensive/p10_sum/", sp, "_", model, "_lowFilter_sum_CV.png")
    in.rast.filename.CA = paste0("data_v2/4_maxent_outputs/comprehensive/", sp, "/", "lowFilter/p10/p10_", sp, "_", model, "_monthly_CA.tif")
    in.rast.filename.CV = paste0("data_v2/4_maxent_outputs/comprehensive/", sp, "/", "lowFilter/p10/p10_", sp, "_", model, "_monthly_CV.tif")
...
speciesPerMonthHist = function(sp, crop_poly = c("CA",  "CV")){
    ...
    input.filenames.hist = list.files(
        paste0("data_v2/4_maxent_outputs/comprehensive/", sp, "/lowFilter/p10/p10_hist"),
        full.names=T
    )
    input.filenames.MIROC45 = list.files(
        paste0("data_v2/4_maxent_outputs/comprehensive/", sp, "/lowFilter/p10/p10_MIROC45"),
        full.names=T
    )
    input.filenames.MIROC85 = list.files(
        paste0("data_v2/4_maxent_outputs/comprehensive/", sp, "/lowFilter/p10/p10_MIROC85"),
        full.names=T
    )
    ...
    output.filename = paste0('data_v2/5_figs/lowFilter/comprehensive/ha_histograms/p10_', sp, '_monthly_barplots_', crop_poly, '.png')
    ...
    output.csv.filename = paste0('data_v2/6_tables/lowFilter/comprehensive/ha_histograms/p10_', sp, '_suitable_ha.csv')
    write_csv(ha_per_month.all_wider, output.csv.filename)
}

dir.create('data_v2/5_figs/lowFilter/comprehensive/ha_histograms/', recursive = T)
dir.create('data_v2/6_tables/lowFilter/comprehensive/ha_histograms/', recursive = T)
map(
    .x = names,
...
predictionAggregate = function(sp, monthly_dist_path){
    in.path = paste0("data_v2/4_maxent_outputs/comprehensive/",
                      sp, 
                      "/lowFilter/", monthly_dist_path)
    in.training.filename = paste0('data_v2/3_swd/monthly/comprehensive/training_', sp, '_soil200cm_lowFilter_monthly_comprehensive.csv')
    in.files = list.files(
        in.path,
        full.names=T
    )
    ...
    out.filename = paste0("data_v2/4_maxent_outputs/comprehensive/figs/prediction_aggregate/", monthly_dist_path, "/", sp, '.png')
    dir.create(paste0("data_v2/4_maxent_outputs/comprehensive/figs/prediction_aggregate/", monthly_dist_path))
    ggplot() + 
        geom_spatraster(
            data = in.rast
        ) +
...
predictionPerMonth = function(sp, monthly_dist_path){
    out.filename = paste0('data_v2/5_figs/lowFilter/comprehensive/monthly_suitability/', monthly_dist_path, '/', sp, '_monthly_suitability.png')
    in.path = paste0("data_v2/4_maxent_outputs/comprehensive//",
                     sp, 
                     "/lowFilter/", monthly_dist_path)
    in.swd.filename = paste0('data_v2/3_swd/monthly/comprehensive/swd_', sp, '_occ_soil200cm_lowFilter_monthly_comprehensive.csv')
    in.files = list.files(
        in.path,
        full.names=T
    )
...
dir.create("data_v2/5_figs/lowFilter/comprehensive/monthly_suitability/monthly_dist_hist", recursive = T)
dir.create("data_v2/5_figs/lowFilter/comprehensive/monthly_suitability/monthly_dist_MIROC45", recursive = T)
dir.create("data_v2/5_figs/lowFilter/comprehensive/monthly_suitability/monthly_dist_MIROC85", recursive = T)
...
envDistribution = function(sp){
    output.filepath = paste0('data_v2/5_figs/lowFilter/comprehensive/env_distribution/monthly_comprehensive/')
...
write_csv(aucs, "data_v2/6_tables/lowFilter/comprehensive/aucs.csv")

# Permutation importance --------------------------------------------------

getVImp = function(sp){
    mod.filename = paste0('data_v2/4_maxent_outputs/comprehensive/', sp, '/lowFilter/model/', sp, '_final_sdm.rds')
    sp_pretty = sp %>% 
        str_replace("_", ". ") %>% 
        str_to_sentence()
...
var_imps = map(
    ...
    bind_rows()
write_csv(var_imps, 'data_v2/6_tables/lowFilter/comprehensive/perm_var_imps.csv')
