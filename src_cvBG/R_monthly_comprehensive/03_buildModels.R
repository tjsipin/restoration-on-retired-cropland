...
library(SDMtune)
tidymodels_prefer()
dir.create('data_cvBG/4_maxent_outputs/comprehensive/tuning/', recursive = T)
set.seed(123)
names = c(
    'a_menziesii',
    'a_intermedia',
...
map(
    names,
    function(sp){
        enm_eval.filename = paste0('data_cvBG/4_maxent_outputs/comprehensive/tuning/', sp, "_tuned_args_lowFilter.rds")
        enm_eval_res.filename = paste0('data_cvBG/4_maxent_outputs/comprehensive/tuning/', sp, "_tuned_args_res_lowFilter.rds")
        ...
        cat("Processing:", sp, "\n")
        swd <- read_csv(paste0('data_cvBG/3_swd/monthly/comprehensive/training_', sp, '_soil200cm_lowFilter_monthly_comprehensive.csv')) %>% 
...
map(
    names,
    function(sp){
        res.filename = paste0('data_cvBG/4_maxent_outputs/comprehensive/tuning/', sp, "_tuned_args_res_lowFilter.rds")
        args.filename = paste0("data_cvBG/4_maxent_outputs/comprehensive/tuning/", sp, "_finalModelArgs_lowFilter.rds")
...
map(
    ...
    function(sp){
        ...
        print(sp)    
        # Read in all points
        training <- read_csv(paste0('data_cvBG/3_swd/monthly/comprehensive/training_', sp, '_soil200cm_lowFilter_monthly_comprehensive.csv')) %>% 
            mutate(tdiff = tmx - tmn) %>% 
            select(
                lon, lat, month, year, 
                aet, tmx, ppt_winter_sum, tmx_summer_mean, tdiff,
        ...
        args = readRDS(paste0("data_cvBG/4_maxent_outputs/comprehensive/tuning/", sp, "_finalModelArgs_lowFilter.rds"))
        
        ## Path to save results
        out.path <- paste0('data_cvBG/4_maxent_outputs/comprehensive/', sp, '/lowFilter/model_training/')
...
aucs = map(
    ...
    function(sp){
        training = read_csv(paste0('data_cvBG/3_swd/monthly/comprehensive/training_', sp, '_soil200cm_lowFilter_monthly_comprehensive.csv'))
        testing = read_csv(paste0('data_cvBG/3_swd/monthly/comprehensive/testing_', sp, '_soil200cm_lowFilter_monthly_comprehensive.csv'))
        model = readRDS(paste0('data_cvBG/4_maxent_outputs/comprehensive/', sp, '/lowFilter/model_training/', sp, '_training_sdm.rds'))
        best_rm = readRDS(paste0('data_cvBG/4_maxent_outputs/comprehensive/tuning/', sp, '_finalModelArgs_lowFilter.rds'))[4] %>% 
            str_split_i('=', 2)
...
map(
    ...
    function(sp){
        ...
        # Read in all points
        training <- read_csv(
            paste0("data_cvBG/3_swd/monthly/comprehensive/training_", sp, "_soil200cm_lowFilter_monthly_comprehensive.csv")
        ) %>%
            mutate(tdiff = tmx - tmn) %>% 
            mutate(drclass = as.factor(drclass)) %>%
            select(
        ...
        testing <- read_csv(
            paste0("data_cvBG/3_swd/monthly/comprehensive/testing_", sp, "_soil200cm_lowFilter_monthly_comprehensive.csv")
        ) %>%
            mutate(tdiff = tmx - tmn) %>% 
            mutate(drclass = as.factor(drclass)) %>%
            select(
        ...
        ## Path to save results
        out.path <- paste0('data_cvBG/4_maxent_outputs/comprehensive/', sp, '/lowFilter/model/')
        dir.create(out.path, recursive=T)
