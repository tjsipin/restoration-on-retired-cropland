...
dir.create('data_cvBG/3_swd/monthly/selective', recursive=T)
source('src_lowFilter_v4_cvBG/util/env_extract_monthly.R')

##Extract for even months background set
dir.create('data_cvBG/3_swd/monthly/comprehensive', recursive=T)
set.seed(123)
map(
    1:nrow(spp_occ_back),
    function(i){
        ...
        out.filename = paste0('data_cvBG/3_swd/monthly/comprehensive/swd_', spp, '_', occ_back, '_soil200cm_lowFilter_monthly_comprehensive.csv')
        # if(file.exists(out.filename)) return(NULL)
        if(occ_back=='occ'){
            points.filename = paste0('data/1_occ/combined_spp_occ/', spp, '_lowFilter.csv')
        } else{
            points.filename = paste0("data_cvBG/2_background/back_", spp, "_5km_lowFilter_even.csv")
        }
...
map(
    ...
    function(sp){
        print(sp)
        out.dir = paste0('data_cvBG/3_swd/monthly/comprehensive')
