...
library(SDMtune)
tidymodels_prefer()
dir.create('data_v4_cvBG/4_maxent_outputs/wy/tuning/', recursive = T)
set.seed(123)
...
map(
    ...
    function(sp){
        set.seed(123)
        enm_eval.filename = paste0('data_v4_cvBG/4_maxent_outputs/wy/tuning/', sp, "_tuned_args_lowFilter.rds")
        enm_eval_res.filename = paste0('data_v4_cvBG/4_maxent_outputs/wy/tuning/', sp, "_tuned_args_res_lowFilter.rds")
        ...
        # }
        cat("Processing:", sp, "\n")
        swd <- read_csv(paste0('data_v4_cvBG/3_swd/wy/', '/training_', sp, '_soil200cm_lowFilter_wy.csv')) %>% 
            select(
                x, y, month, year, 
...
best_model_args = map(
    names,
    function(sp){
        res.filename = paste0('data_v4_cvBG/4_maxent_outputs/wy/tuning/', sp, "_tuned_args_res_lowFilter.rds")
        args.filename = paste0("data_v4_cvBG/4_maxent_outputs/wy/tuning/", sp, "_finalModelArgs_lowFilter.rds")
...
map(
    ...
    function(sp){
        ...
        # Read in all points
        training <- read_csv(
            paste0("data_v4_cvBG/3_swd/wy/", "/training_", sp, "_soil200cm_lowFilter_wy.csv")
        ) %>%
            mutate(
                drclass=as.factor(drclass),
                salinity=as.factor(salinity),
        ...
        args = readRDS(paste0("data_v4_cvBG/4_maxent_outputs/wy/tuning/", sp, "_finalModelArgs_lowFilter.rds"))

        ## Path to save results
        out.path <- paste0('data_v4_cvBG/4_maxent_outputs/wy/', sp, '/lowFilter/model_training/')
...
variable_importance = map(
    names,
    function(sp){
        model = readRDS(paste0('data_v4_cvBG/4_maxent_outputs/wy/', sp, '/lowFilter/model_training/', sp, '_training_sdm.rds'))
...
map(
    ...
    function(sp){
        ...
        testing <- read_csv(
            paste0("data_v4_cvBG/3_swd/wy/", "/testing_", sp, "_soil200cm_lowFilter_wy.csv")
        ) %>%
            mutate(
                drclass=as.factor(drclass),
                salinity=as.factor(salinity),
        ...
        ## Path to save results
        out.path <- paste0('data_v4_cvBG/4_maxent_outputs/wy//', sp, '/lowFilter/model/')
        dir.create(out.path, recursive=T)
