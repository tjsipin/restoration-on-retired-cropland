...
p10Visualize = function(sp, model){
    ...
    out.dir = paste0("data_MAIN/5_figs/lowFilter/monthly/p10_sum/")
    dir.create(out.dir, recursive=T)
    out.png.filename.CA = paste0("data_MAIN/5_figs/lowFilter/monthly/p10_sum/", sp, "_", model, "_lowFilter_sum_CA.png")
    out.png.filename.CV = paste0("data_MAIN/5_figs/lowFilter/monthly/p10_sum/", sp, "_", model, "_lowFilter_sum_CV.png")
    in.rast.filename.CA = paste0("data_MAIN/4_maxent_outputs/monthly/", sp, "/", "lowFilter/p10/p10_", sp, "_", model, "_monthly_CA.tif")
    in.rast.filename.CV = paste0("data_MAIN/4_maxent_outputs/monthly/", sp, "/", "lowFilter/p10/p10_", sp, "_", model, "_monthly_CV.tif")
...
dir.create('data_MAIN/5_figs/lowFilter/monthly/ha_histograms/', recursive = T)
dir.create('data_MAIN/6_tables/lowFilter/monthly/ha_histograms/', recursive = T)
...
speciesPerMonthHist = function(sp, crop_poly = c("CA",  "CV")){
    ...
    input.filenames.hist = list.files(
        paste0("data_MAIN/4_maxent_outputs/monthly/", sp, "/lowFilter/p10/p10_hist"),
        full.names=T
    )
    input.filenames.MIROC45 = list.files(
        paste0("data_MAIN/4_maxent_outputs/monthly/", sp, "/lowFilter/p10/p10_MIROC45"),
        full.names=T
    )
    input.filenames.MIROC85 = list.files(
        paste0("data_MAIN/4_maxent_outputs/monthly/", sp, "/lowFilter/p10/p10_MIROC85"),
        full.names=T
    )
    ...
    ha_per_month.hist = map(
        ...
        function(i){
            ...
            out.filename = paste0('data_MAIN/6_tables/lowFilter/monthly/ha_histograms/hist_', sp, '_', i, '_', crop_poly, '.rds')
    ...
    ha_per_month.MIROC45 = map(
        ...
        function(i){
            ...
            out.filename = paste0('data_MAIN/6_tables/lowFilter/monthly/ha_histograms/RCP45_', sp, '_', i, '_', crop_poly, '.rds')
            if(nrow(readRDS(out.filename))!=0){
                out = readRDS(out.filename)
                return(out)
            }
    ...
    ha_per_month.MIROC85 = map(
        ...
        function(i){
            ...
            out.filename = paste0('data_MAIN/6_tables/lowFilter/monthly/ha_histograms/RCP85_', sp, '_', i, '_', crop_poly, '.rds')
            if(nrow(readRDS(out.filename))!=0){
                out = readRDS(out.filename)
                return(out)
            }
    ...
    output.csv.filename = paste0('data_MAIN/6_tables/lowFilter/monthly/ha_histograms/p10_', sp, '_suitable_ha_', crop_poly, '.csv')
...
#         input.filename = paste0('data_MAIN/6_tables/lowFilter/monthly/ha_histograms/p10_', sp, '_suitable_ha.csv')
...
plotSpeciesPerMonthHist = function(sp, crop_poly){
    ...
    in.filename=paste0('data_MAIN/6_tables/lowFilter/monthly/ha_histograms/p10_', sp, '_suitable_ha_', crop_poly, '.csv')
    ...
    output.filename = paste0('data_MAIN/5_figs/lowFilter/monthly/ha_histograms/p10_', sp, '_monthly_barplots_', crop_poly, '.png')
...
# Figure 5 ----------------------------------------------------------------
##03/10/26 Need to condense the plots according to Ashley 

fig5_data = map(
    c('c_pungens', 'p_ciliata', 'a_polycarpa'),
    function(sp){
        in.filename = paste0('data_MAIN/6_tables/lowFilter/monthly/ha_histograms/p10_', sp, '_suitable_ha_CV.csv')
        in.file=read_csv(in.filename)
        ...
        ha_per_month.all = in.file %>% 
            ...
            rename(mon = name, num_ha=value) %>% 
            mutate(mon = factor(mon, levels=str_to_sentence(months), ordered=T))
        return(ha_per_month.all)
    }
) %>% 
    bind_rows()

ggplot(data=fig5_data) +
    geom_histogram(
        aes(
            x = mon,
            y = num_ha,
    ...
    labs(
        ...
        y = 'Hectares',
        fill = "Years (and Climate Scenario)",
        title = paste0('Suitability across the Great Valley (hectares)')
    ) +
    theme(
        text = element_text(
            family="Times New Roman", size = 18
...
ggsave('data_MAIN/5_figs/lowFilter/fig5.png', scale = 2, width = 7, height = 4, units = 'in', bg = 'white')

predictionAggregate = function(sp, monthly_dist_path){
    in.path = paste0("data_MAIN/4_maxent_outputs/monthly/",
                      sp, 
                      "/lowFilter/", monthly_dist_path)
    in.training.filename = paste0('data_MAIN/3_swd/monthly/training_', sp, '_soil200cm_lowFilter_monthly.csv')
    in.files = list.files(
        in.path,
        full.names=T
    )
    ...
    out.filename = paste0("data_MAIN/4_maxent_outputs/monthly/figs/prediction_aggregate/", monthly_dist_path, "/", sp, '.png')
    dir.create(paste0("data_MAIN/4_maxent_outputs/monthly/figs/prediction_aggregate/", monthly_dist_path))
    ggplot() + 
        geom_spatraster(
            data = in.rast
        ) +
...
predictionPerMonth = function(sp, monthly_dist_path){
    out.filename = paste0('data_MAIN/5_figs/lowFilter/monthly/monthly_suitability/', monthly_dist_path, '/', sp, '_monthly_suitability.png')
    in.path = paste0("data_MAIN/4_maxent_outputs/monthly//",
                     sp, 
                     "/lowFilter/", monthly_dist_path)
    in.swd.filename = paste0('data_MAIN/3_swd/monthly/swd_', sp, '_occ_soil200cm_lowFilter_monthly.csv')
    in.files = list.files(
        in.path,
        full.names=T
    )
...
dir.create("data_MAIN/5_figs/lowFilter/monthly/monthly_suitability/monthly_dist_hist", recursive = T)
dir.create("data_MAIN/5_figs/lowFilter/monthly/monthly_suitability/monthly_dist_MIROC45", recursive = T)
dir.create("data_MAIN/5_figs/lowFilter/monthly/monthly_suitability/monthly_dist_MIROC85", recursive = T)
...
predictionPerMonth(sp = 'a_intermedia', monthly_dist_path = 'monthly_dist_hist')
predictionPerMonth(sp = 'c_lasiophyllus', monthly_dist_path = 'monthly_dist_hist')
predictionPerMonth(sp = 'l_californica', monthly_dist_path = 'monthly_dist_hist')
predictionPerMonth(sp = 'l_gracilis', monthly_dist_path = 'monthly_dist_hist')
...
predictionPerMonth(sp = 'a_intermedia', monthly_dist_path = 'monthly_dist_MIROC45')
predictionPerMonth(sp = 'c_lasiophyllus', monthly_dist_path = 'monthly_dist_MIROC45')
predictionPerMonth(sp = 'l_californica', monthly_dist_path = 'monthly_dist_MIROC45')
predictionPerMonth(sp = 'l_gracilis', monthly_dist_path = 'monthly_dist_MIROC45')
...
predictionPerMonth(sp = 'a_intermedia', monthly_dist_path = 'monthly_dist_MIROC85')
predictionPerMonth(sp = 'c_lasiophyllus', monthly_dist_path = 'monthly_dist_MIROC85')
predictionPerMonth(sp = 'l_californica', monthly_dist_path = 'monthly_dist_MIROC85')
predictionPerMonth(sp = 'l_gracilis', monthly_dist_path = 'monthly_dist_MIROC85')

envDistribution = function(sp){
    output.filepath = paste0('data_MAIN/5_figs/lowFilter/monthly/env_distribution/monthly/')
    dir.create(output.filepath, recursive=T)
    output.filename = paste0(output.filepath, sp, '_envDistribution.png')
    ...
    ggplot(training_long) +
        ...
        theme_classic() +
        labs(
            title = "monthly monthly model environmental density distributions",
            subtitle = pretty_sp,
            x = "",
            y = '',
            color = ''
...
write_csv(aucs, "data_MAIN/6_tables/lowFilter/monthly/aucs.csv")

# Permutation importance --------------------------------------------------

getVImp = function(sp){
    mod.filename = paste0('data_MAIN/4_maxent_outputs/monthly/', sp, '/lowFilter/model/', sp, '_final_sdm.rds')
    sp_pretty = sp %>% 
        str_replace("_", ". ") %>% 
        str_to_sentence()
    ...
    permutation.imp = tibble(
        ...
        select(sp, aet, tmx, ppt_winter_sum, tmx_summer_mean, tdiff, cec, drclass, om, ph, salinity)
...
var_imps = map(
    ...
    bind_rows()
write_csv(var_imps, 'data_MAIN/6_tables/lowFilter/monthly/perm_var_imps.csv')
...
ggplot(var_imps_long) +
    ...
    theme_light() +
    labs(y='', x='', title = 'Variable permutation importance (Monthly model)') +
    theme(
        text=element_text(family='times', size=12),
        strip.text=element_text(face='italic')
    )
ggsave('data_MAIN/5_figs/lowFilter/monthly/figS6_varImp.png', width = 3200, height = 2400, units = 'px', bg='white')
