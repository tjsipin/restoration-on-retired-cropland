...
## Read in fxn
source('src_lowFilter_v5_50km/util/pred_agg.R')
...
map(
    ...
    function(sp){
        ...
        model = readRDS(paste0("data_v5_50km/4_maxent_outputs/agg/", sp,
                               "/lowFilter/model/",
                               sp, "_final_sdm.rds"))
        out.path = paste0("data_v5_50km/4_maxent_outputs/agg/",
                          sp,
...
getThresholdRasts = function(sp, model_years=c("2000_2023", "MIROC45_2070_2099", "MIROC85_2070_2099")){
    ...
    training.filename = paste0('data_v5_50km/3_swd/agg/training_', sp, '_soil200cm_lowFilter_agg.csv')
    testing.filename = paste0('data_v5_50km/3_swd/agg/testing_', sp, '_soil200cm_lowFilter_agg.csv')
    model.filename = paste0("data_v5_50km/4_maxent_outputs/agg/", sp, "/lowFilter/model/", sp, "_final_sdm.rds")
    ...
    input.filenames = list.files(
        paste0('data_v5_50km/4_maxent_outputs/agg/', sp, '/lowFilter/', monthly_dist_str),
        full.names = T,
        pattern='.tif'
    )
    ...
    dir.create(paste0('data_v5_50km/4_maxent_outputs/agg/', sp, '/lowFilter/p10/'), recursive=T, showWarnings=F)
    output.filename = paste0('data_v5_50km/4_maxent_outputs/agg/', sp, '/lowFilter/p10/', '/p10_', sp, '_', model_years, '_agg.tif')
...
sumThresholdRasts = function(model){
    out.dir = paste0("data_v5_50km/4_maxent_outputs/agg/p10/")
    dir.create(out.dir, recursive=T)
    out.rast.filename.CA = paste0("data_v5_50km/4_maxent_outputs/agg/p10/", model, "_lowFilter_sum_CA.tif")
    out.rast.filename.CV = paste0("data_v5_50km/4_maxent_outputs/agg/p10/", model, "_lowFilter_sum_CV.tif")
    #Read in p10 rasters
    input_rast.filenames = list.files(
        paste0("data_v5_50km/4_maxent_outputs/agg/"),
