...
dir.create('data_v5_50km/5_figs/lowFilter/wy', recursive=T)
...
distributionSpPlotWithoutLegend = function(sp){
    print(sp)
    output.filename = paste0("data_v5_50km/5_figs/lowFilter/wy/fig2_", sp, "_2000_2023.png")
    input.filename = paste0("data_v5_50km/4_maxent_outputs/wy/", sp, "/lowFilter/monthly_dist_hist/", sp, "_2000_2023_wy.tif")
...
#Extract legend
sp='l_gracilis'
input.legend.filename = paste0("data_v5_50km/4_maxent_outputs/wy/", sp, "/lowFilter/monthly_dist_hist/", sp, "_2000_2023_wy.tif")
...
ggsave('data_v5_50km/5_figs/lowFilter/fig2.png', width = 2800, height = 2180, units = "px", bg = 'white')
...
species <- names
wy_root <- "data_v5_50km/4_maxent_outputs/wy"
...
change_layer_for <- function(spp, scenario = c("MIROC45","MIROC85")) {
    ...
    p10_dir  <- file.path(wy_root, spp, "lowFilter", "p10")
    f_hist   <- file.path(p10_dir, sprintf("p10_%s_2000_2023_wy.tif", spp))
    f_fut    <- file.path(p10_dir, sprintf("p10_%s_%s_2070_2099_wy.tif", spp, scenario))
...
#             legend.justification = "center", 
#             legend.spacing       = unit(7.0, 'cm'),
#             legend.key.width     = unit(1.0, "cm"),
#             legend.key.height    = unit(0.6, "cm"),
#             legend.title         = element_text(hjust = 0.5, size = 20),
#             legend.text          = element_text(size = 14, margin = margin(l = 8, r = 30)),
#             legend.box.margin    = margin(t = 0, r = 24, b = 0, l = 24),
#             plot.margin          = margin(t = 0, r = 24, b = 0, l = 24),
#             text                 = element_text(family = "Times")
#         ) +
#         guides(fill = guide_legend(
#             title.position = "top", title.hjust = 0.5,
#             ncol = 4, byrow = FALSE, label.position = "right",
...
p_45_5 = ggRastLayer(5, chg=chg_45, gv=gv45, model='RCP4.5')
...
p_45_cowplot = cowplot::plot_grid(
    ...
    ncol = 6, rel_widths = 1, align = "h"
)
p_45_title = ggdraw() +
    draw_label_theme("Current to MIROC RCP 4.5 Distribution (Water year model)", element = 'plot.title', title_size = 20)
...
ggsave(
    'data_v5_50km/5_figs/lowFilter/fig3_RCP45.png',
    plot = p_45_final,
...
p_85_5 = ggRastLayer(5, chg=chg_85, gv=gv85, model='RCP8.5')
...
p_85_title = ggdraw() +
    draw_label_theme("Current to MIROC RCP 8.5 Distribution (Water year model)", element = 'plot.title', title_size = 20)
...
ggsave(
    'data_v5_50km/5_figs/lowFilter/fig3_RCP85.png',
    plot = p_85_final,
...
fig3_p45_cowplot = cowplot::plot_grid(
    p_45_1, p_45_2, p_45_3, p_45_4,
    p_45_5, p_45_6, p_45_7, p_45_8, p_45_9, p_45_10, 
    ncol = 10, rel_widths = 1, align = "h"
)
fig3_p85_cowplot = cowplot::plot_grid(
    p_85_1, p_85_2, p_85_3, p_85_4,
    p_85_5, p_85_6, p_85_7, p_85_8, p_85_9, p_85_10,
    ncol = 10, rel_widths = 1, align = "h"
)

fig3_combine = cowplot::plot_grid(
    p_45_title,
    fig3_p45_cowplot,
    p_85_title,
    fig3_p85_cowplot,
    horizontal_legend_fig3,
    ncol = 1, rel_heights = c(0.1, 1, 0.01, 1, 0.1), align = 'w'
)
fig3_final = cowplot::plot_grid(
    fig3_combine, vertical_legend_fig3,
    ncol = 2, rel_widths = c(1, 0.1), align = 'w'
)

ggsave(
    'data_v5_50km/5_figs/lowFilter/fig3.png',
    plot = fig3_combine,
    width = 4800, height = 3200, units = "px",
    bg = "white", limitsize = FALSE
)
...
p10Panel <- function(model, show_compass = FALSE) {
    ...
    base_dir = 'data_v5_50km/4_maxent_outputs/wy/p10/'
...
final_path <- paste0('data_v5_50km/5_figs/lowFilter/fig4.png')
...
ca_rast = ifel(is.na(ca_rast), NA, 1) %>%
    as.polygons()
total_ca_ha = ca_rast %>% expanse('ha') #4675858

suitable_hectares_CA = map(
    names,
    function(sp){
        print(sp)
        input.filename.hist = paste0('data_v5_50km/4_maxent_outputs/wy/', sp, '/lowFilter/p10/', '/p10_', sp, '_2000_2023_wy.tif')
        input.filename.RCP45 = paste0('data_v5_50km/4_maxent_outputs/wy/', sp, '/lowFilter/p10/', '/p10_', sp, '_MIROC45_2070_2099_wy.tif')
        input.filename.RCP85 = paste0('data_v5_50km/4_maxent_outputs/wy/', sp, '/lowFilter/p10/', '/p10_', sp, '_MIROC85_2070_2099_wy.tif')
        ...
        # num_ha.hist = terra::global(input.rast.hist, fun='sum', na.rm=T)$sum*0.027 # convert pixel to hectare
        # num_ha.RCP45 = terra::global(input.rast.RCP45, fun='sum', na.rm=T)$sum*0.027 # convert pixel to hectare
        # num_ha.RCP85 = terra::global(input.rast.RCP85, fun='sum', na.rm=T)$sum*0.027 # convert pixel to hectare
        sp_pretty = sp %>%
            str_to_sentence() %>%
            str_replace("_", ". ")
...
dir.create('data_v5_50km/6_tables/lowFilter/wy', recursive=T)
write_csv(suitable_hectares_CA, "data_v5_50km/6_tables/lowFilter/wy/suitable_hectares_wy_CA.csv")
suitable_hectares_CA = read_csv("data_v5_50km/6_tables/lowFilter/wy/suitable_hectares_wy_CA.csv")
...
dir.create('data_v5_50km/6_tables/lowFilter/wy')
write_csv(suitable_hectares_CV, "data_v5_50km/6_tables/lowFilter/wy/suitable_hectares_wy_CV.csv")
...
aucs = map(
    ...
    function(sp){
        ...
        testing = read_csv(paste0('data_v5_50km/3_swd/wy/testing_', sp, '_soil200cm_lowFilter_wy.csv'))
        model = readRDS(paste0('data_v5_50km/4_maxent_outputs/wy/', sp, '/lowFilter/model_training/', sp, '_training_sdm.rds'))
        best_rm = readRDS(paste0('data_v5_50km/4_maxent_outputs/wy/tuning/', sp, '_finalModelArgs_lowFilter.rds'))[4] %>%
            str_split_i('=', 2)
...
write_csv(aucs, 'data_v5_50km/6_tables/lowFilter/wy/aucs_lowFilter.csv')


# Permutation importance --------------------------------------------------

getVImp = function(sp){
    mod.filename = paste0('data_v5_50km/4_maxent_outputs/wy/', sp, '/lowFilter/model/', sp, '_final_sdm.rds')
    sp_pretty = sp %>%
        str_replace("_", ". ") %>%
        str_to_sentence()
    ...
    permutation.imp = tibble(
        ...
        select(
            ...
            ppt10, ppt11, ppt12, ppt1, ppt2, ppt3,
            ppt4, ppt5, ppt6, ppt7, ppt8, ppt9,
            ppt_max_month, ppt_min_month,
            tmx, tmx_summer_mean, tdiff,
            tmx_max_month, tmx_min_month,
            cec, om, ph, drclass, salinity
        )
...
var_imps = map(
    ...
    bind_rows()
write_csv(var_imps, 'data_v5_50km/6_tables/lowFilter/wy/perm_var_imps_lowFilter.csv')
...
ggplot(var_imps_long) +
    ...
    theme_light() +
    labs(y='', x='', title = 'Variable permutation importance (Water year model)') +
    theme(
        text=element_text(family='times', size=12),
        strip.text=element_text(face='italic')
    )
ggsave('data_v5_50km/5_figs/lowFilter/figS6_varImp.png', width = 3200, height = 2400, units = 'px', bg='white')
...
current_p10_rasts = map(
    ...
    function(sp){
        list.files(
            paste0('data_v5_50km/4_maxent_outputs/wy/', sp, '/lowFilter/p10/'),
            pattern = '_2000_2023_wy.tif',
            full.names=T
        )
    }
) %>% 
...
suitable_sp_count.current = current_p10_rasts %>% 
    expanse(byValue=T, unit='ha') %>% 
    select(Number_of_species=value, Ha=area) %>% 
    mutate(Model = "2000 to 2023")
...
suitable_sp_count.rcp45 = rcp45_p10_rasts %>% 
    expanse(byValue=T, unit='ha') %>% 
    select(Number_of_species=value, Ha=area) %>% 
    mutate(Model = "2070 to 2099 (MIROC-RCP 4.5)")
rcp85_p10_rasts = map(
    names,
    function(sp){
        list.files(
...
suitable_sp_count.rcp85 = rcp85_p10_rasts %>% 
    expanse(byValue=T, unit='ha') %>% 
    select(Number_of_species=value, Ha=area) %>% 
    mutate(Model = "2070 to 2099 (MIROC-RCP 8.5)")

suitable_sp_count.empty = tibble(
    Model = c(rep('2000 to 2023', 11), rep('2070 to 2099 (MIROC-RCP 4.5)', 11), rep('2070 to 2099 (MIROC-RCP 8.5)', 11)),
    Number_of_species = rep(0:10, 3)
)

suitable_sp_count.all = suitable_sp_count.empty %>% 
    full_join(
        rbind(
            suitable_sp_count.current,
            suitable_sp_count.rcp45,
            suitable_sp_count.rcp85
        )
    ) %>% 
    mutate(Ha = ifelse(is.na(Ha), 0, Ha) %>% round(1))


ggplot(data=suitable_sp_count.all, aes(fill=Model, group=Model)) +
    geom_histogram(
        aes(x=Number_of_species, y = Ha), 
        color = 'darkgrey',
        stat='identity',
        position = 'dodge'
    ) +
    ...
    scale_y_continuous(labels = scales::comma, limits = c(0, 1550000)) +
...
ggsave("data_v5_50km/5_figs/lowFilter/figS5.png", width = 2040, height = 1440, units='px', scale = 2)
suitable_sp_count.all_wider = suitable_sp_count.all %>% 
    pivot_wider(names_from='Number_of_species', values_from=c('Ha'), names_prefix = 'Number of species = ', values_fill = 0)
write_csv(suitable_sp_count.all_wider, "data_v5_50km/6_tables/lowFilter/wy/tableS9_nsp_ha.csv")


p10_rasts = map(
    names,
    function(sp=.x){
        file=paste0('data_v5_50km/4_maxent_outputs/wy/', sp, '/lowFilter/p10/p10_', sp, '_2000_2023_wy.tif')
        r = rast(file) %>% 
            crop(cv_rast, mask=T) %>% 
            mutate(sum=ifelse(sum==0, 'Not suitable', 'Suitable'))
        return(r)
    }
) %>% 
    rast()
names(p10_rasts) = paste0(names) %>% str_replace("_", ". ") %>% str_to_sentence()
# p10_rasts=ifel(p10_rasts==1, 'Suitable', 'Not suitable')
ggplot() + 
    geom_spatraster( 
        data=p10_rasts
    ) +
    scale_fill_wa_d('sound_sunset', na.translate = FALSE) +
    theme_void() +
    theme(
    ...
    labs(title='P10 suitability under current conditions', fill='') +
    facet_wrap(~lyr, nrow=2) 
ggsave('data_v5_50km/5_figs/lowFilter/figS2_p10_facet.png', width=3200, height=2800, units='px', bg='white')

p10_sums = paste0('data_v5_50km/4_maxent_outputs/wy/p10/2000_2023_lowFilter_sum_CV.tif') %>% 
    rast()
...
p10_sums_r = p10_sums %>% 
    mutate(
        bins=case_when(
            ...
            factor(ordered=T, levels=c('0', '1', '2-5', '6-7', '8', '9', '10'))
    ) %>%
    select(bins) 

p10_sums_r_RCP45 = rast('data_v5_50km/4_maxent_outputs/wy/p10/MIROC45_2070_2099_lowFilter_sum_CV.tif') %>% 
    mutate(
        bins=case_when(
            sum == 0 ~ '0',
            sum == 1 ~ '1',
    ...
    select(bins) 
p10_sums_r_RCP85 = rast('data_v5_50km/4_maxent_outputs/wy/p10/MIROC85_2070_2099_lowFilter_sum_CV.tif') %>% 
    mutate(
        bins=case_when(
            sum == 0 ~ '0',
            sum == 1 ~ '1',
...
places_CA = tigris::places(state = 'CA') %>% 
    vect() %>% 
    project('epsg:3310') %>% 
    crop(cv_rast)
counties_CA = tigris::counties(state='CA') %>% 
    vect() %>% 
    project('epsg:3310') %>% 
    crop(cv_rast)
...
library(tmap)
tmap_mode('view')
map_cv = tm_shape(places_CA) + 
    tm_polygons() +
    tm_shape(counties_CA) + 
    tm_polygons() +
    tm_shape(p10_sums_r) +
    tm_raster(col_alpha = 0.7) +
    tm_shape(p10_sums_r_RCP45) +
    tm_raster(col_alpha = 0.7) +
    tm_shape(p10_sums_r_RCP85) +
    tm_raster(col_alpha = 0.7)

p=ggplot() + 
    # geom_spatraster(data=p10_sums_r) +
    geom_sf(data=st_as_sf(places_CA), aes(fill=NAME))
...
fig1_spOccs = map(
    ...
    function(sp){
            ...
            'data/1_occ/combined_spp_occ/', sp, '_lowFilter.csv'
        )
        dat = read_csv(fi) %>% 
            mutate(sp = sp)
    }
    ...
    mutate(
        sp_pretty = sp %>% 
            str_replace('_', '. ') %>% 
            str_to_sentence() %>% 
            factor(, levels = names_pretty, ordered=T)
    ) %>% 
    vect(geom=c('lon', 'lat'), crs='epsg:4326')
fig1_spOccs_minConvHull = fig1_spOccs %>% 
    convHull(by='sp_pretty') %>% 
    terra::buffer(50000)

ggplot(mapping = aes(color=sp_pretty, fill=sp_pretty)) + 
    geom_spatvector(data=CA, fill='grey', color='grey') +
    geom_spatvector(data=fig1_spOccs, size = 0.5) +
    geom_spatvector(data=fig1_spOccs_minConvHull, alpha=0.2, size=1.25) +
    scale_color_discrete() +
    scale_fill_discrete() +
    facet_wrap(~sp_pretty) +
    theme_void() +
    theme(
        strip.text = element_text(face='italic', family = 'times', size = 15),
        legend.position = 'none',
        strip.clip = 'off'
    ) 
ggsave('data/5_figs/figS1.png', width=3200, height=2400, units='px', bg='white')

# Appendix ----------------------------------------------------------------
# suitable_hectares_CV = read_csv("data_v5_50km/6_tables/lowFilter/wy/suitable_hectares_agg_CV.csv")
...
#     ) %>% 
#     kable_classic(html_font = 'Times') %>% 
#     save_kable('data_v5_50km/6_tables/lowFilter/wy/suitable_hectares_agg_CA.png')
# 
# 
...
#     ) %>% 
#     kable_classic(html_font = 'Times') %>% 
#     save_kable('data_v5_50km/6_tables/lowFilter/wy/suitable_hectares_agg_CV.png')
# 
# 
...
#         tmx.filename = paste0('data_v5_50km/4_maxent_outputs/wy/', sp, '/lowFilter/model/plots/species_tmx.dat')
...
#         str_to_lower()
# 
#     output.filename = paste0('data_v5_50km/5_figs/lowFilter/wy/tmx_opt_diff_', sp, '_', model, '_lowFilter.png')
# 
#     if(model == "hist"){
#         model.filepath = "data_v5_50km/bcm/bcmv8_historic/monthly_avgs/"
#     } else if(model == "RCP4.5"){
#         model.filepath = "data_v5_50km/bcm/bcm_future/MIROC45/resampled/"
#     } else if(model == "RCP8.5"){
#         model.filepath = "data_v5_50km/bcm/bcm_future/MIROC85/resampled/"
#     }
# 
...
#     if(future == "RCP4.5"){
#         future.filepath = "data_v5_50km/bcm/bcm_future/MIROC45/resampled/"
#     } else if(future == "RCP8.5"){
#         future.filepath = "data_v5_50km/bcm/bcm_future/MIROC85/resampled/"
#     }
# 
#     env_files.map = map(
...
#                 "data_v5_50km/bcm/bcmv8_historic/monthly_avgs/",
#                 pattern = env_var,
#                 full.names = T
#             )
...
#         }
#     )
#     output.filename = paste0('data_v5_50km/5_figs/lowFilter/wy/env_diff/diff_future_', future, '_lowFilter.png')
...
#     output.filepath = paste0('data_v5_50km/figs/env_distribution/wy/')
#     dir.create(output.filepath, recursive=T)
#     output.filename = paste0(output.filepath, sp, '_envDistribution_lowFilter.png')
#     training = read_csv(paste0('data_v5_50km/3_swd/wy/training_', sp, '_soil200cm_lowFilter_agg.csv')) %>% 
#         mutate(
#             tdiff = tmx - tmn,
