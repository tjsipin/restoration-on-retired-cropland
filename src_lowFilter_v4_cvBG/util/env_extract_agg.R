...
extractEnvAgg = function(sp, relevant_months){
    ...
    back_filename = paste0('data_cvBG/2_background/back_', sp, '_5km_lowFilter_even.csv')
    back = read_csv(back_filename) %>% 
        mutate(sp = sp, presence = 0) %>% 
        select(sp, lon, lat, month, year, presence)
    
    out.dir = 'data_cvBG/3_swd/agg'
    
    out.filename = paste0('data_cvBG/3_swd/agg/', 'swd_', sp, '_soil200cm_lowFilter_agg.csv')
