...
extractEnvWY = function(sp){
    ...
    occ_filename = paste0('data/1_occ/combined_spp_occ/', sp, '_highFilter.csv')
    occ = read_csv(occ_filename) %>% 
        mutate(sp = sp, presence = 1) %>% 
        select(sp, lon, lat, month, year, presence)
    
    back_filename = paste0('data_v5_50km_highFilter/2_background/back_', sp, '_5km_highFilter_even.csv')
