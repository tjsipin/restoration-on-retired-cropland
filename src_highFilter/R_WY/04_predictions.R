...
## Read in fxn
source('src_highFilter/util/pred_wy.R')
...
map(
    ...
    function(sp){
        ...
        model = readRDS(paste0("data_highFilter/4_maxent_outputs/wy/", sp,
                               "/highFilter/model/",
                               sp, "_final_sdm.rds"))
        out.path = paste0("data_highFilter/4_maxent_outputs/wy/",
                          sp,
...
###Output not run for RCP 4.5 and RCP 8.5
#RCP4.5
map(
    names,
    function(sp){
...
getThresholdRasts = function(sp, model_years=c("2000_2023", "MIROC45_2070_2099", "MIROC85_2070_2099")){
    ...
    training.filename = paste0('data_highFilter/3_swd/wy/training_', sp, '_soil200cm_highFilter_wy.csv')
    testing.filename = paste0('data_highFilter/3_swd/wy/testing_', sp, '_soil200cm_highFilter_wy.csv')
    model.filename = paste0("data_highFilter/4_maxent_outputs/wy/", sp, "/highFilter/model/", sp, "_final_sdm.rds")
    ...
    input.filenames = list.files(
        paste0('data_highFilter/4_maxent_outputs/wy/', sp, '/highFilter/', monthly_dist_str),
        full.names = T,
        pattern='.tif'
    )
    ...
    dir.create(paste0('data_highFilter/4_maxent_outputs/wy/', sp, '/highFilter/p10/'), recursive=T, showWarnings=F)
    output.filename = paste0('data_highFilter/4_maxent_outputs/wy/', sp, '/highFilter/p10/', '/p10_', sp, '_', model_years, '_wy.tif')
...
sumThresholdRasts = function(model){
    out.dir = paste0("data_highFilter/4_maxent_outputs/wy/p10/")
    dir.create(out.dir, recursive=T)
    out.rast.filename.CA = paste0("data_highFilter/4_maxent_outputs/wy/p10/", model, "_highFilter_sum_CA.tif")
    out.rast.filename.CV = paste0("data_highFilter/4_maxent_outputs/wy/p10/", model, "_highFilter_sum_CV.tif")
    #Read in p10 rasters
    input_rast.filenames = list.files(
        paste0("data_highFilter/4_maxent_outputs/wy/"),
