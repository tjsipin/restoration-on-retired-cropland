...
## Read in fxn
source('src_lowFilter_v2/util/pred_month.R')
...
map(
    ...
    function(sp){
        ...
        model = readRDS(paste0("data_v2/4_maxent_outputs/comprehensive/", sp,
                               "/lowFilter/model/",
                               sp, "_final_sdm.rds"))
        out.path = paste0("data_v2/4_maxent_outputs/comprehensive/",
                          sp,
...
map(
    ...
    function(sp){
        ## Read in Maxent model
        print(paste0("Working on ", sp))
        model = readRDS(paste0("data_v2/4_maxent_outputs/comprehensive/", sp, "/lowFilter/model/", sp, "_final_sdm.rds"))
        out.path = paste0("data_v2/4_maxent_outputs/comprehensive//",
                          sp,
...
getThresholdRasts = function(sp, model_years=c("2000_2023", "MIROC45_2070_2099", "MIROC85_2070_2099")){
    print(paste0(sp, "\n", model_years))
    dir.create(paste0('data_v2/4_maxent_outputs/comprehensive/', sp, '/lowFilter/p10/'), recursive=T, showWarnings=F)
    output.filename.CV = paste0('data_v2/4_maxent_outputs/comprehensive/', sp, '/lowFilter/p10/', '/p10_', sp, '_', model_years, '_monthly_CV.tif')
    output.filename.CA = paste0('data_v2/4_maxent_outputs/comprehensive/', sp, '/lowFilter/p10/', '/p10_', sp, '_', model_years, '_monthly_CA.tif')
    
    #Read in training data and model
    training.filename = paste0('data_v2/3_swd/monthly/comprehensive/training_', sp, '_soil200cm_lowFilter_monthly_comprehensive.csv')
    testing.filename = paste0('data_v2/3_swd/monthly/comprehensive/testing_', sp, '_soil200cm_lowFilter_monthly_comprehensive.csv')
    model.filename = paste0("data_v2/4_maxent_outputs/comprehensive/", sp, "/lowFilter/model/", sp, "_final_sdm.rds")
    ...
    input.filenames = list.files(
        paste0('data_v2/4_maxent_outputs/comprehensive/', sp, '/lowFilter/', monthly_dist_str),
        full.names = T
    )
    map(
        input.filenames,
    ...
    thresh.filenames = list.files(
        paste0("data_v2/4_maxent_outputs/comprehensive/", sp, "/lowFilter/p10/"),
