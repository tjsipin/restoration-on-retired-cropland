...
library(SDMtune)
tidymodels_prefer()
dir.create('data_5km/4_maxent_outputs/agg/tuning/', recursive = T)
set.seed(123)
...
map(
    names,
    function(sp){
        enm_eval.filename = paste0('data_5km/4_maxent_outputs/agg/tuning/', sp, "_tuned_args_lowFilter.rds")
        enm_eval_res.filename = paste0('data_5km/4_maxent_outputs/agg/tuning/', sp, "_tuned_args_res_lowFilter.rds")

        cat("Processing:", sp, "\n")
        swd <- read_csv(paste0('data_5km/3_swd/agg/', '/training_', sp, '_soil200cm_lowFilter_agg.csv')) %>%
            select(
                x, y, month, year,
...
map(
    names,
    function(sp){
        res.filename = paste0('data_5km/4_maxent_outputs/agg/tuning/', sp, "_tuned_args_res_lowFilter.rds")
        args.filename = paste0("data_5km/4_maxent_outputs/agg/tuning/", sp, "_finalModelArgs_lowFilter.rds")
...
map(
    ...
    function(sp){
        ...
        # Read in all points
        training <- read_csv(
            paste0("data_5km/3_swd/agg/", "/training_", sp, "_soil200cm_lowFilter_agg.csv")
        ) %>%
            mutate(
                drclass=as.factor(drclass),
                salinity=as.factor(salinity),
        ...
        args = readRDS(paste0("data_5km/4_maxent_outputs/agg/tuning/", sp, "_finalModelArgs_lowFilter.rds"))

        ## Path to save results
        out.path <- paste0('data_5km/4_maxent_outputs/agg/', sp, '/lowFilter/model_training/')
...
map(
    ...
    function(sp){
        ...
        testing <- read_csv(
            paste0("data_5km/3_swd/agg/", "/testing_", sp, "_soil200cm_lowFilter_agg.csv")
        ) %>%
            mutate(
                drclass=as.factor(drclass),
                salinity=as.factor(salinity),
        ...
        ## Path to save results
        out.path <- paste0('data_5km/4_maxent_outputs/agg//', sp, '/lowFilter/model/')
        dir.create(out.path, recursive=T)
