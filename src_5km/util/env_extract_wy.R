...
extractEnvWY = function(sp){
    ...
    back_filename = paste0('data_v2/2_background/back_', sp, '_5km_lowFilter_even.csv')
    back = read_csv(back_filename) %>% 
        mutate(sp = sp, presence = 0) %>% 
        select(sp, lon, lat, month, year, presence)
    
    out.dir = 'data_v2/3_swd/wy'
    
    out.filename = paste0('data_v2/3_swd/wy/', 'swd_', sp, '_soil200cm_lowFilter_wy.csv')
