...
library(SDMtune)
tidymodels_prefer()
dir.create('data_5km/4_maxent_outputs/monthly/tuning/', recursive = T)
set.seed(123)
...
map(
    names,
    function(sp){
        enm_eval.filename = paste0('data_5km/4_maxent_outputs/monthly/tuning/', sp, "_tuned_args_lowFilter.rds")
        enm_eval_res.filename = paste0('data_5km/4_maxent_outputs/monthly/tuning/', sp, "_tuned_args_res_lowFilter.rds")
        ...
        # }
        cat("Processing:", sp, "\n")
        swd <- read_csv(paste0('data_5km/3_swd/monthly//training_', sp, '_soil200cm_lowFilter_monthly.csv')) %>% 
...
map(
    names,
    function(sp){
        res.filename = paste0('data_5km/4_maxent_outputs/monthly/tuning/', sp, "_tuned_args_res_lowFilter.rds")
        args.filename = paste0("data_5km/4_maxent_outputs/monthly/tuning/", sp, "_finalModelArgs_lowFilter.rds")
...
map(
    ...
    function(sp){
        ...
        args = readRDS(paste0("data_5km/4_maxent_outputs/monthly/tuning/", sp, "_finalModelArgs_lowFilter.rds"))
        
        ## Path to save results
        out.path <- paste0('data_5km/4_maxent_outputs/monthly/', sp, '/lowFilter/model_training/')
...
map(
    ...
    function(sp){
        ...
        # Read in all points
        training <- read_csv(
            paste0("data_5km/3_swd/monthly//training_", sp, "_soil200cm_lowFilter_monthly.csv")
        ) %>%
            mutate(tdiff = tmx - tmn) %>% 
            mutate(drclass = as.factor(drclass)) %>%
            select(
        ...
        testing <- read_csv(
            paste0("data_5km/3_swd/monthly//testing_", sp, "_soil200cm_lowFilter_monthly.csv")
        ) %>%
            mutate(tdiff = tmx - tmn) %>% 
            mutate(drclass = as.factor(drclass)) %>%
            select(
        ...
        ## Path to save results
        out.path <- paste0('data_5km/4_maxent_outputs/monthly/', sp, '/lowFilter/model/')
        dir.create(out.path, recursive=T)
